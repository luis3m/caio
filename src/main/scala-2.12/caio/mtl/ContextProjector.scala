...
trait ContextProjector {
  ...
  implicit def projectStateful[M[_], A]
    (implicit MSP: StateProjection[M, A]): Stateful[M, A] =
    MSP.MS

  implicit def projectAsk[M[_], A]
    (implicit AP: AskProjection[M, A]): InvariantAsk[M, A] =
    AP.A
  ...
  implicit def askAskProjection[M[_], A, B]
    (implicit AA: InvariantAsk[M, A], M: Mixer[A, B], ev: A =:!= B): AskProjection[M, B] =
    AskProjection {
      new MixedAsk[M, A, B](AA)
    }

  implicit def stateStateProjection[M[_], A, B]
    (implicit MS: Stateful[M, A], M: Mixer[A, B]): StateProjection[M, B] =
    StateProjection {
