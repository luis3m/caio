...
abstract class CaioConcurrent[C, V, L] extends CaioSpawn[C, V, L] with GenConcurrent[Caio[C, V, L, *], Throwable] { 
  final def ref[A](a: A): Caio[C, V, L, Ref[Caio[C, V, L, *], A]] =
    Caio.apply(Ref.unsafe[Caio[C, V, L, *], A](a))

  final def deferred[A]: Caio[C, V, L, Deferred[Caio[C, V, L, *], A]] =
    Caio.apply(Deferred.unsafe[Caio[C, V, L, *], A])
}

object CaioConcurrent {
  def apply[C, V, L]: CaioConcurrent[C, V, L] =
