...
abstract class CaioTemporal[C, V, L] extends CaioConcurrent[C, V, L] with GenTemporal[Caio[C, V, L, *], Throwable] with CaioClock[C, V, L] {
  final def sleep(time: FiniteDuration): Caio[C, V, L, Unit] =
    Caio.liftIO(IO.sleep(time))
}

object CaioTemporal {
  def apply[C, V, L]: CaioTemporal[C, V, L] =
