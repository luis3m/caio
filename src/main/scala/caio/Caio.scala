...
import caio.std.{CaioAsync, CaioClock, CaioConcurrent, CaioFunctor, CaioMonad, CaioMonadCancel, CaioSpawn, CaioTemporal}
...
sealed trait Caio[-C, +V, +L, +A] {
  final def <>[C1 <: C, L1 >: L, V2, A1 >: A](that: => Caio[C1, V2, L1, A1]): Caio[C1, V2, L1, A1] =
    orElse(that)
  ...
  final def <&[C1 <: C, V1 >: V, L1 >: L](that: Caio[C1, V1, L1, Any]): Caio[C1, V1, L1, A] =
    both(that).map { case (a, _) => a }

  final def <&>[C1 <: C, V1 >: V, L1 >: L, B](that: Caio[C1, V1, L1, B]): Caio[C1, V1, L1, (A, B)] =
    both(that)

  final def &>[C1 <: C, V1 >: V, L1 >: L, B](that: Caio[C1, V1, L1, B]): Caio[C1, V1, L1, B] =
    both(that).map { case (_, b) => b }

  final def >>=[C1 <: C, V1 >: V, L1 >: L, B](f: A => Caio[C1, V1, L1, B]): Caio[C1, V1, L1, B] =
    flatMap(f)

  final def <+>[C1 <: C, V1 >: V, L1 >: L, B](that: => Caio[C1, V1, L1, B]): Caio[C1, V1, L1, Either[A, B]] =
    orElseEither(that)
  ...
  final def absolve[V1 >: V, B](implicit ev: A <:< Either[V1, B]): Caio[C, V1, L, B] =
    flatMap(either => Caio.fromEitherFailure(ev(either)))

  final def background: Resource[
    Caio[C @uncheckedVariance, V @uncheckedVariance, L @uncheckedVariance, *] @uncheckedVariance,
    Caio[C, V, L, OutcomeCaio[C @uncheckedVariance, V @uncheckedVariance, L @uncheckedVariance, A @uncheckedVariance]]
  ] =
    CaioSpawn[C, V, L].background(this)

  final def bracket[C1 <: C, V1 >: V, L1 >: L, A1 >: A, B]
    (use: A1 => Caio[C1, V1, L1, B])
    (release: A1 => Caio[C1, V1, L1, Unit]): Caio[C1, V1, L1, B] =
    bracketCase(use){ case (a, _) => release(a) }

  final def bracketCase[C1 <: C, V1 >: V, L1 >: L, A1 >: A, B]
    (use: A1 => Caio[C1, V1, L1, B])
    (release: (A1, OutcomeCaio[C1, V1, L1, B]) => Caio[C1, V1, L1, Unit]): Caio[C1, V1, L1, B] =
    CaioMonadCancel[C1, V1, L1].bracketCase(this)(use)(release)

  final def both[C1 <: C, V1 >: V, L1 >: L, B](other: => Caio[C1, V1, L1, B]): Caio[C1, V1, L1, (A, B)] =
    CaioAsync[C1, V1, L1].both(this, other)

  final def censor[L1 >: L](f: L1 => L1)(implicit M: Monoid[L1]): Caio[C, V, L1, A] =
    Caio.CensorCaio(this, f, () => M.empty)

  final def censorWithDefault[L1 >: L](default: => L1)(f: L1 => L1): Caio[C, V, L1, A] =
    Caio.CensorCaio(this, f, () => default)

  final def clear: Caio[C, V, Nothing, A] =
    Caio.ClearCaio(this)

  final def contramap[C0](f: C0 => C): Caio[C0, V, L, A] =
    Caio.getContext[C0].flatMap(c0 => provideContext(f(c0)))

  final def delayBy(duration: FiniteDuration): Caio[C, V, L, A] =
  ...
  final def eventually: Caio[C, Nothing, L, A] =
    orElse(eventually)
  ...
  final def fold[V1 >: V, A1 >: A, B](failure: NonEmptyList[V1] => B, success: A1 => B): Caio[C, Nothing, L, B] =
    foldM[C, V1, Nothing, L, A1, B](f => Caio.pure(failure(f)), s => Caio.pure(success(s)))

  final def foldM[C1 <: C, V1 >: V, V2, L1 >: L, A1 >: A, B](failure: NonEmptyList[V1] => Caio[C1, V2, L1, B], success: A1 => Caio[C1, V2, L1, B]): Caio[C1, V2, L1, B] =
    Caio.CatamorphismCaio(this, failure, success)

  final def forceR[C1 <: C, V1 >: V, L1 >: L, B](other: Caio[C1, V1, L1, B]): Caio[C1, V1, L1, B] =
    either.attempt *> other

  final def foreverM: Caio[C, V, L, Nothing] =
    CaioMonad[C, V, L].foreverM[A, Nothing](this)

  final def guarantee[C1 <: C, V1 >: V, L1 >: L, A1 >: A](finalizer: Caio[C1, V1, L1, Unit]): Caio[C1, V1, L1, A1] =
    guaranteeCase[C1, V1, L1, A1](_ => finalizer)

  final def guaranteeCase[C1 <: C, V1 >: V, L1 >: L, A1 >: A](finalizer: OutcomeCaio[C1, V1, L1, A1] => Caio[C1, V1, L1, Unit]): Caio[C1, V1, L1, A1] =
    CaioMonadCancel[C1, V1, L1].guaranteeCase[A1](this)(finalizer)

  final def head[B](implicit ev: A <:< List[B]): Caio[C, V, L, Option[B]] =
  ...
  final def listen(implicit M: Monoid[L @uncheckedVariance]): Caio[C, V, L, (A, L)] =
    Caio.ListenCaio[C, V, L, A](this, () => M.empty)

  final def listenWithDefault[L1 >: L](default: => L1): Caio[C, V, L1, (A, L1)] =
    Caio.ListenCaio[C, V, L1, A](this, () => default)

  final def localContext[C1 <: C](f: C1 => C1): Caio[C1, V, L, A] =
    Caio.LocalContextCaio(this, f)

  final def map[B](f: A => B): Caio[C, V, L, B] =
    Caio.BindCaio[C, V, L, A, B](this, a => Caio.PureCaio(f(a)))

  final def memoize: Caio[C, V, L, Caio[C, V, L, A]] =
    CaioConcurrent[C, V, L].memoize(this)

  final def onCancel[C1 <: C, V1 >: V, L1 >: L](finalizer: Caio[C1, V1, L1, Unit]): Caio[C1, V1, L1, A] =
  ...
  final def orElse[C1 <: C, L1 >: L, V2, A1 >: A](that: => Caio[C1, V2, L1, A1]): Caio[C1, V2, L1, A1] =
    handleFailuresWith[C1, V, V2, L1, A1](_ => that)

  final def orElseEither[C1 <: C, V1 >: V, L1 >: L, B](that: => Caio[C1, V1, L1, B]): Caio[C1, V1, L1, Either[A, B]] =
    map(Left.apply).orElse(that.map(Right.apply))

  final def provideContext(c: C): Caio[Any, V, L, A] =
    Caio.RefIOCaio[L, A](ref => Caio.run[C, V, L, A](this, c, ref))

  final def race[C1 <: C, V1 >: V, L1 >: L, B](that: Caio[C1, V1, L1, B]): Caio[C1, V1, L1, Either[A, B]] =
    CaioSpawn[C1, V1, L1].race(this, that)

  final def racePair[C1 <: C, V1 >: V, L1 >: L, A1 >: A, B](that: Caio[C1, V1, L1, B])
    : Caio[C1, V1, L1, Either[(OutcomeCaio[C1, V1, L1, A1], FiberCaio[C1, V1, L1, B]), (FiberCaio[C1, V1, L1, A1], OutcomeCaio[C1, V1, L1, B])]] =
    CaioSpawn[C1, V1, L1].racePair[A1, B](this, that)
  ...
  final def start: Caio[C, V, L, FiberCaio[C @uncheckedVariance, V @uncheckedVariance, L @uncheckedVariance, A @uncheckedVariance]] =
    CaioSpawn[C, V, L].start[A](this)
  ...
  final def timeoutTo[C1 <: C, V1 >: V, L1 >: L, A1 >: A](duration: FiniteDuration, fallback: Caio[C1, V1, L1, A1]): Caio[C1, V1, L1, A1] =
    CaioTemporal[C1, V1, L1].timeoutTo(this, duration, fallback)

  final def timeout(duration: FiniteDuration): Caio[C, V, L, A] =
    CaioTemporal[C, V, L].timeout(this, duration)

  final def timed: Caio[C, V, L, (FiniteDuration, A)] =
    CaioClock[C, V, L].timed(this)

  final def uncancelable: Caio[C, V, L, A] =
    Caio.uncancelable[C, V, L, A](_ => this)

  final def unit: Caio[C, V, L, Unit] =
    as(())
  ...
  final def run(c: C): IO[A] =
    Caio.run[C, V, L, A](this, c, Ref.unsafe[IO, Option[L]](None))

  final def run(implicit ev: C @uncheckedVariance =:= Any): IO[A] =
    Caio.run[Any, V, L, A](this.asInstanceOf[Caio[Any, V, L, A]], (), Ref.unsafe[IO, Option[L]](None))

  final def runFail(c: C): IO[Either[NonEmptyList[V], A]] =
    Caio.runFail[C, V, L, A](this, c, Ref.unsafe[IO, Option[L]](None))

  final def runFail(implicit ev: C @uncheckedVariance =:= Any): IO[Either[NonEmptyList[V], A]] =
    Caio.runFail[Any, V, L, A](this.asInstanceOf[Caio[Any, V, L, A]], (), Ref.unsafe[IO, Option[L]](None))

  final def runContext(c: C): IO[(C @uncheckedVariance, Option[L], Either[ErrorOrFailure[V @uncheckedVariance], A])] =
    Caio.foldIO[C, V, L, A](this, c, Ref.unsafe[IO, Option[L]](None)).map {
...
object Caio {
  ...
  def async[C, V, L, A](k: (Either[Throwable, A] => Unit) => Caio[C, V, L, Option[Caio[C, V, L, Unit]]]): Caio[C, V, L, A] =
    CaioAsync[C, V, L].async[A](k)

  def async_[A](k: (Either[Throwable, A] => Unit) => Unit): Caio[Any, Nothing, Nothing, A] =
    CaioAsync[Any, Nothing, Nothing].async_[A](k)

  def bracketFull[C, V, L, A, B]
    (acquire: Poll[Caio[C, V, L, *]] => Caio[C, V, L, A])
    (use: A => Caio[C, V, L, B])
    (release: (A, OutcomeCaio[C, V, L, B]) => Caio[C, V, L, Unit]): Caio[C, V, L, B] =
    CaioMonadCancel[C, V, L].bracketFull(acquire)(use)(release)

  def both[C, V, L, A, B](`this`: Caio[C, V, L, A], that: Caio[C, V, L, B]): Caio[C, V, L, (A, B)] =
    `this`.both(that)

  def canceled: Caio[Any, Nothing, Nothing, Unit] =
    liftIO(IO.canceled)

  def cede: Caio[Any, Nothing, Nothing, Unit] =
    liftIO(IO.cede)

  def defer[C, V, L, A](thunk: => Caio[C, V, L, A]): Caio[C, V, L, A] =
    delay(thunk).flatten

  def deferred[C, V, L, A]: Caio[C, Nothing, Nothing, Deferred[Caio[C, V, L, *], A]] =
    delay(Deferred.unsafe[Caio[C, V, L, *], A])

  def delay[A](thunk: => A): Caio[Any, Nothing, Nothing, A] =
    apply(thunk)

  def executionContext: Caio[Any, Nothing, Nothing, ExecutionContext] =
  ...
  def monotonic: Caio[Any, Nothing, Nothing, FiniteDuration] =
    liftIO(IO.monotonic)
  ...
  def parTraverseN[C, V, L, T[_]: Traverse, A, B](n: Int)(ta: T[A])(f: A => Caio[C, V, L, B]): Caio[C, V, L, T[B]] =
    asyncForCaio[C, V, L].parTraverseN(n)(ta)(f)

  def parSequenceN[C, V, L, T[_]: Traverse, A](n: Int)(tma: T[Caio[C, V, L, A]]): Caio[C, V, L, T[A]] =
  ...
  def race[C, V, L, A, B](fa: Caio[C, V, L, A], fb: Caio[C, V, L, B]): Caio[C, V, L, Either[A, B]] =
    fa.race(fb)

  def realTime: Caio[Any, Nothing, Nothing, FiniteDuration] =
    liftIO(IO.realTime)

  def ref[C, V, L, A](a: A): Caio[Any, Nothing, Nothing, Ref[Caio[C, V, L, *], A]] =
    delay(Ref.unsafe[Caio[C, V, L, *], A](a))
  ...
  def sleep(duration: FiniteDuration): Caio[Any, Nothing, Nothing, Unit] =
    CaioTemporal[Any, Nothing, Nothing].sleep(duration)

  def some[A](a: A): Caio[Any, Nothing, Nothing, Option[A]] =
    pure(Some(a))

  def tell[L: Monoid](l: L): Caio[Any, Nothing, L, Unit] =
    TellCaio[L](l)

  def traverse[C, V, L, T[_]: Traverse, A, B](ta: T[A])(f: A => Caio[C, V, L, B]): Caio[C, V, L, T[B]] =
    Traverse[T].traverse(ta)(f)

  def uncancelable[C, V, L, A](body: Poll[Caio[C, V, L, *]] => Caio[C, V, L, A]): Caio[C, V, L, A] =
  ...
  def unique: Caio[Any, Nothing, Nothing, Unique.Token] =
    liftIO(IO.unique)

  implicit def alignForCaio[C, V, L]: Align[Caio[C, V, L, *]] =
    new Align[Caio[C, V, L, *]] {
      val functor: Functor[Caio[C, V, L, *]] =
        CaioFunctor[C, V, L]
  ...
  implicit def asyncForCaio[C, V, L]: Async[Caio[C, V, L, *]] =
    CaioAsync[C, V, L]
  ...
  implicit def parallelForCaio[C, V, L]: Parallel.Aux[Caio[C, V, L, *], Par[C, V, L, *]] =
  ...
  private[caio] def run[C, V, L, A](caio: Caio[C, V, L, A], c: C, ref: Ref[IO, Option[L]]): IO[A] =
  ...
  private[caio] def runFail[C, V, L, A](caio: Caio[C, V, L, A], c: C, ref: Ref[IO, Option[L]]): IO[Either[NonEmptyList[V], A]] =
    foldIO[C, V, L, A](caio, c, ref).map {
      case FoldCaioSuccess(_, _, a) =>
        Right(a)
      case FoldCaioFailure(_, _, head, tail) =>
  ...
  private type Continuation = (Any, Any) => Caio[Any, Any, Any, Any]
  private type ErrorRecovery = (Any, Throwable) => Caio[Any, Any, Any, Any]
  private type FailureRecovery = (Any, NonEmptyList[Any]) => Caio[Any, Any, Any, Any]

  private sealed trait Handler
  private final case class OnSuccess(f: Continuation) extends Handler
  private final case class OnFailure(f: FailureRecovery) extends Handler
  private final case class OnError(f: ErrorRecovery) extends Handler
  private final case class OnSuccessAndFailure(f: FailureRecovery, s: Continuation) extends Handler

  def tryOrError(value: => Caio[Any, Any, Any, Any]): Caio[Any, Any, Any, Any] =
    try value
    catch { case NonFatal(ex) => ErrorCaio(ex) }

  @tailrec private def nextHandler(fs: List[Handler]): Option[(Continuation, List[Handler])] =
    fs match {
      case OnSuccess(c) :: cs => Some((c, cs))
      case OnSuccessAndFailure(_, c) :: cs => Some((c, cs))
      case _ :: cs => nextHandler(cs)
      case Nil => None
    }

  @tailrec private def nextErrorHandler(fs: List[Handler]): Option[(ErrorRecovery, List[Handler])] =
    fs match {
  ...
  @tailrec private def nextFailureHandler(fs: List[Handler]): Option[(FailureRecovery, List[Handler])] =
    fs match {
      case OnFailure(c) :: cs => Some((c, cs))
      case OnSuccessAndFailure(c, _) :: cs => Some((c, cs))
      case _ :: cs => nextFailureHandler(cs)
      case Nil => None
    }

  private[caio] def foldIO[C, V, L, A](caio: Caio[C, V, L, A], c: C, ref: Ref[IO, Option[L]]): IO[FoldCaioPure[C, V, L, A]] = {
    def safeFold(caio: Caio[Any, Any, Any, Any], c: Any, handlers: List[Handler]): IO[FoldCaioPure[Any, Any, Any, Any]] = {
      @tailrec def foldCaio(caio: Caio[Any, Any, Any, Any], c: Any, handlers: List[Handler]): IO[FoldCaioPure[Any, Any, Any, Any]] =
        caio match {
          ...
          case BindCaio(caio: IOCaio[_], f) =>
            caio.f().redeemWith(
              e => safeFold(ErrorCaio(e), c, handlers),
              a => safeFold(tryOrError(f(a)), c, handlers)
            )
          ...
          case caio: IOCaio[_] =>
            ...
            caio.f().redeemWith(
              e => safeFold(ErrorCaio(e), c, handlers),
              a => safeFold(PureCaio(a), c, handlers)
            )

          case RefIOCaio(f) =>
            //The IO monad will bring this back into stack safety
            f(ref.asInstanceOf[Ref[IO, Option[Any]]]).redeemWith(
              e => safeFold(ErrorCaio(e), c, handlers),
              a => safeFold(PureCaio(a), c, handlers)
            )
              ...
              case scala.util.Success(foldIO) =>
                foldIO.flatMap {
                  ...
                  case FoldCaioFailure(c, _, head, tail) =>
                    safeFold(FailureCaio(head, tail), c, handlers)
          ...
          case CatamorphismCaio(source, failure, success) =>
            foldCaio(source, c, OnSuccessAndFailure((_, e) => failure(e), (_, a) => success(a)) :: handlers)
          ...
          case tellCaio @ TellCaio(l2) =>
            foldCaio(IOCaio(ref.update(opt => Some(tellCaio.monoid.combine(opt.getOrElse(tellCaio.monoid.empty), l2).asInstanceOf[L]))), c, handlers)

          case ListenCaio(source, default) =>
            foldCaio(source, c,  OnSuccess((_, a) => IOCaio(ref.get.map(opt => a -> opt.getOrElse(default())))) :: handlers)

          case CensorCaio(source, f, default) =>
            foldCaio(source, c, OnSuccess((_, a) => BindCaio(IOCaio(ref.update(opt => Some(f(opt.getOrElse(default())).asInstanceOf[L]))), (_: Any) => PureCaio(a))) :: handlers)
    ...
    IO.defer(safeFold(caio.asInstanceOf[Caio[Any, Any, Any, Any]], c, Nil)).asInstanceOf[IO[FoldCaioPure[C, V, L, A]]]
  }
  ...
  final private[caio] case class RefIOCaio[L, +A](f: Ref[IO, Option[L]] => IO[A]) extends Caio[Any, Nothing, L, A]

  final private[caio] case class KleisliCaio[C, V, L, +A](kleisli: (C, Ref[IO, Option[L]]) => IO[FoldCaioPure[C, V, L, A]]) extends Caio[C, V, L, A]

  final private case class BindCaio[-C, +V, +L, E, +A](source: Caio[C, V, L, E], f: E => Caio[C, V, L, A]) extends Caio[C, V, L, A]

  final private case class CatamorphismCaio[-C, V, +V2, +L, A, B](source: Caio[C, V, L, A], failure: NonEmptyList[V] => Caio[C, V2, L, B], success: A => Caio[C, V2, L, B]) extends Caio[C, V2, L, B]
  ...
  final private case class TellCaio[L](l: L)(implicit val monoid: Monoid[L]) extends Caio[Any, Nothing, L, Unit]

  final private case class ListenCaio[-C, +V, +L, +A](source: Caio[C, V, L, A], default: () => L) extends Caio[C, V, L, (A, L)]

  final private case class CensorCaio[-C, +V, L, +A](source: Caio[C, V, L, A], f: L => L, default: () => L) extends Caio[C, V, L, A]

  final private case class ClearCaio[-C, +V, L, +A](source: Caio[C, V, L, A]) extends Caio[C, V, Nothing, A]
