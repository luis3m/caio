...
sealed trait Caio[-C, +V, +L, +A] {
  ...
  final def background[C1 <: C, V1 >: V, L1 >: L, A1 >: A]: Resource[Caio[C1, V1, L1, *], Caio[C1, V1, L1, OutcomeCaio[C1, V1, L1, A1]]] =
  ...
  final def delayBy[L1 >: L](duration: FiniteDuration): Caio[C, V, L1, A] =
  ...
  final def listen[L1 >: L](implicit M: Monoid[L1]): Caio[C, V, L1, (A, L1)] =
    Caio.ListenCaio[C, V, L1, A](this, () => M.empty)
  ...
  final def memoize[L1 >: L]: Caio[C, V, L1, Caio[C, V, L1, A]] =
  ...
  final def provideContext[L1 >: L](c: C): Caio[Any, V, L1, A] =
    Caio.RefIOCaio[L1, A](ref => Caio.run[C, V, L1, A](this, c, ref))

  final def race[C1 <: C, V1 >: V, L1 >: L, B](other: Caio[C1, V1, L1, B]): Caio[C1, V1, L1, Either[A, B]] =
    CaioSpawn[C1, V1, L1].race(this, other)

  final def racePair[C1 <: C, V1 >: V, L1 >: L, A1 >: A, B](other: Caio[C1, V1, L1, B])
  ...
  final def sleep[C1 <: C, V1 >: V, L1 >: L](duration: FiniteDuration): Caio[C1, V1, L1, Unit] =
     CaioTemporal[C1, V1, L1].sleep(duration)

  final def start[C1 <: C, V1 >: V, L1 >: L, A1 >: A]: Caio[C1, V1, L1, FiberCaio[C1, V1, L1, A1]] =
  ...
  final def timeout[C1 <: C, V1 >: V, L1 >: L](duration: FiniteDuration): Caio[C1, V1, L1, A] =
  ...
  final def run[C1 <: C](implicit ev: C1 =:= Any): IO[A] =
  ...
  final def runFail[C1 <: C](implicit ev: C1 =:= Any): IO[Either[NonEmptyList[V], A]] =
    Caio.runFail[Any, V, L, A](this.asInstanceOf[Caio[Any, V, L, A]], (), Ref.unsafe[IO, Option[L]](None))

  final def runContext[C1 <: C, V1 >: V](c: C1): IO[(C1, Option[L], Either[ErrorOrFailure[V1], A])] =
    Caio.foldIO[C1, V1, L, A](this, c, Ref.unsafe[IO, Option[L]](None)).map {
...
object Caio {
  ...
  def async_[C, V, L, A](k: (Either[Throwable, A] => Unit) => Unit): Caio[C, V, L, A] =
  ...
  def sleep[C, V, L](duration: FiniteDuration): Caio[C, V, L, Unit] =
    ...
    def safeFold(caio: Caio[Any, Any, Any, Any], c: Any, handlers: List[Handler]): FoldCaio[Any, Any, Any, Any] = {
      @tailrec def foldCaio(caio: Caio[Any, Any, Any, Any], c: Any, handlers: List[Handler]): FoldCaio[Any, Any, Any, Any] =
        caio match {
          ...
          case RefIOCaio(f) =>
            //The IO monad will bring this back into stack safety
            FoldCaioIO(f(ref.asInstanceOf[Ref[IO, Option[Any]]]).redeemWith(
              e => safeFold(ErrorCaio(e), c, handlers).toIO,
              a => safeFold(PureCaio(a), c, handlers).toIO
            ))
          ...
          case CensorCaio(source, f, default) =>
            foldCaio(source, c, OnSuccess((_, a) => MapCaio(IOCaio(ref.update(opt => Some(f(opt.getOrElse(default())).asInstanceOf[L]))), (_: Any) => a)) :: handlers)
